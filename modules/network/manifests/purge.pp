class purge {
	...
	package {'openvswitch-commond':			ensure => purged,}
    ...
    package {'quantum-dhcp-agent':					ensure => purged,}
    package {'quantum-l3':							ensure => purged,}

    package {'quantum-metadata-agent':		ensure => purged,}
    
	file {'reset.interfaces':
		path	=> "/etc/network/interfaces",
		ensure	=> file,
	#	source	=> "puppet:///modules/network/interfaces"
		source	=> "/etc/puppet/modules/network/files/interfaces",
	} 
